...
export class QuestionImportExportModalComponent implements OnInit {
  ...
  async importFile(): Promise<void> {
    ...
    try {
      const result = await this.questionImportService.importFromFile(
        this.selectedFile
      );
      ...
      if (result.success && result.questionSet) {
        ...
        if (duplicates.length > 0) {
          ...
          const resolution =
            this.duplicateResolutionService.autoResolveDuplicates(
              result.questionSet.questions
            );
      ...
      this.logService.log(
        'import',
        'error',
        `Failed to import ${this.selectedFile.name}`
      );
    } finally {
  ...
  confirmImport(): void {
    if (this.importedQuestionSet) {
      ...
      // Close modal and return the question set
      this.activeModal.close({questions: this.importedQuestionSet.questions});
    }
  }
  ...
  exportAsJSON(): void {
    ...
    this.logService.log(
      'export',
      'success',
      `Exported ${this.exportQuestions.length} questions as JSON`
    );
  }
  ...
  exportAsCSV(): void {
    ...
    this.logService.log(
      'export',
      'success',
      `Exported ${this.exportQuestions.length} questions as CSV`
    );
  }
